...
var settings = {
	...
	// disable Upload test for CMIP data nodes
	...
	// set to false because this function relies on getIP.php which actually only locates at speedtest server.
...
function pingTest(done) {
	...
	var doPing = function() {
		...
		xhr[0].open("HEAD", settings.url_ping + url_sep(settings.url_ping) + "r=" + Math.random(), true); // random string to prevent caching
		//use timeout
		xhr[0].timeout = 20000;
		xhr[0].ontimeout = xhr.onerror;
		xhr[0].send();
	}.bind(this);
