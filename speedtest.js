...
Speedtest.prototype = {
  ...
  _checkServerDefinition: function (server) {
    try {
      ...
      // not using Protocol relative url here.
  ...
  selectServer: function (result) {
    ...
    var select = function (serverList, result) {
      ...
      var ping = function (url, result) {
        // no need for cors becauese CMIP node thredds server dont provide Access-Control-Allow-Origin' header as default
        // url += (url.match(/\?/) ? "&" : "?") + "cors=true";
        var xhr = new XMLHttpRequest();
        var t = new Date().getTime();
        xhr.onload = function () {
          //ideally we expect an empty response but some thredds node dont have an empty root page
          if (xhr.getResponseHeader.length > 0) {
      ...
      // jquery ajax jsonp, try to solve CORS problem but blocked by Chrome CORB
      // var ping = function (url, result) {
      ...
      var checkServer = function (server, done) {
        var i = 0;
        server.pingT = -1;
        // not using the same protocol as this website, therfore disable this test/
        // if (server.server.indexOf(location.protocol) == -1) done();
        if (server.server == null) done();
        else {
  ...
  start: function () {
    ...
    if (this._state == 2) {
      ...
      // disable default url assembling, and use custom servers defined in index.html instead of s.setParameter()
      this._settings.url_dl = this._selectedServer.dlURL;
      this._settings.url_ping = this._selectedServer.pingURL;
